...
function SidebarMenu({item}){
    return(
        <div className='flex flex-col space-y-1'>
            <button className='mr-4 w-50 px-4 ml-2 block rounded-md bg-primary2 px-3 py-2 text-md font-medium font-display font-semibold text-white hover:bg-primary1 '><span className='flex items-center space-x-4'>{item.icon} <span>{item.label}</span></span></button>
        </div>
    )
}